...
export default function ChatPage() {
  const { username: currentUsername } = authStore();
  ...
  const handleDeleteMessage = useCallback(async (messageId: number) => {
    try {
      ...
      setAllMessages(prev => prev.filter(msg => msg.message_id !== messageId));
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="flex flex-col h-[calc(100vh-80px)] sm:h-[calc(100vh-88px)]">
        <div className="flex-1 bg-white mx-2 sm:mx-4 my-2 sm:my-4 rounded-lg shadow-sm border border-gray-200 flex flex-col overflow-hidden">
